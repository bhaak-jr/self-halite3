...
class MapCell:
    ...
    def mark_safe(self):
        ...
        Use in conjunction with GameMap.navigate.
        """
        self.ship = None
...
class GameMap:
    ...
    def navigate(self, ship, destination):
        ...
        for direction in self.get_unsafe_moves(ship.position, destination):
            ...
            if not self[target_pos].is_occupied:
                if self[ship.position].halite_amount > ship.halite_amount * 10:
    ...
    def __calc_halite__(self, pos, rec):
        if rec == 0:
            return 0

        sum = 0
        for x in pos.get_surrounding_cardinals():
            sum += self[x].halite_amount
            sum += self.__calc_halite__(x, rec - 1)

        return sum

    def get_best_position(self, pos, next_pos):
        for position in pos.get_surrounding_cardinals():
            if not self[position].is_occupied:
                if self[position].halite_amount + self.__calc_halite__(position, 3) > self[next_pos].halite_amount + self.__calc_halite__(next_pos, 3):
                    next_pos = position
        return next_pos
